 ...
 * <a href="http://www.cs.umass.edu/~mimno/papers/fast-topic-model.pdf">Gibbs Sampler</a> for LDA.
 * This class provides in-sample perplexity estimates to control convergence.
 */
public class Sampler {
    private static Logger logger = Logger.getLogger(Sampler.class);
     ...
     */
    public Sampler(int numberOfTopics,
     ...
     */
    public Sampler disablePerplexityEstimate() {
     ...
     */
    public void sample(int[][] tokensInDocuments, int vocabularySize) {
        sample(tokensInDocuments, vocabularySize, new Random());
    }
     ...
     */
    public void sample(int[][] tokensInDocuments, int vocabularySize, Random random) {
    ...
    public String[][] topicsAndTerms(String[] dictionary){
        String[][] topics = new String[numberOfTopics][];
        for(int topic=0; topic< numberOfTopics; ++topic) { 
            List<String> topTokens = extractTopTokens(topic, dictionary);
            topics[topic] = topTokens.toArray(new String[0]);
        }
        return topics;
    }

    private List<String> extractTopTokens(int tNum, String[] dictionary) {
        List<String> tokens = new ArrayList<String>();
        double [] dst = phi[tNum];
        List<Tuple> tuples = new ArrayList<Tuple>();
        for(int i=0; i <dst.length ; ++i){
            tuples.add(new Tuple(i, dst[i]));
        }
        Collections.sort(tuples);
        for(int i=0; i< tuples.size(); ++i){
            tokens.add(dictionary[tuples.get(i).position]);
        }
        return tokens; 
    }
    static class Tuple implements Comparable<Tuple>{
        ...
        Tuple(int position, double value ) {
            this.value = value;
