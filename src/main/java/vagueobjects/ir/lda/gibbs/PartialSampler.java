...
/**
 * This sampler is used in a streaming environment, when new documents keep
 * coming and we want to extract topics from these documents without re-generating topics across the entire corpus.
 * Instead, topics from existing documents are used to assign topic distribution for new documents.
 */
public class PartialSampler {
    ...
    public double[ ] sampleSingle(int[] tokensInDoc, double[][] phi) {
        int[][] tokensInDocuments = new int[1][];
        tokensInDocuments[0] = tokensInDoc;
        return sample(tokensInDocuments,  phi)[0];
    }
